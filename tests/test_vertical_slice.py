...
def test_create_order_atomic_transaction(setup_db, ddb_client):
    ...
    order = create_order("TENANT_001", "CUS_001", items, "Address")
    ...
    with pytest.raises(ClientError) as exc:
        create_order("TENANT_001", "CUS_001", [{"productId": "PROD_001", "quantity": 10}], "Address")
...
@patch("handlers.ingress.BedrockAdapter.converse")
def test_ingress_lambda_e2e(mock_converse, setup_db):
    ...
    # Mock webhook event with proper signature
    body = json.dumps({
        "entry": [{"changes": [{"value": {"messages": [{"from": "919347761153", "text": {"body": "I need 1 kg basmati"}}]}}]}]
    })
...
def test_ingress_idempotency(mock_converse, setup_db):
    ...
    body = json.dumps({
        "entry": [{"changes": [{"value": {"messages": [{"id": "wamid.idemp123", "from": "919347761153", "text": {"body": "hello"}}]}}]}]
    })
    ...
    assert json.loads(res2["body"])["status"] == "duplicate"
    # Verify Bedrock NOT called again
    assert mock_converse.call_count == 1
